...
import com.zjxtwvf.smarttime.global.Constants;
import com.zjxtwvf.smarttime.utils.AppUtil;
import com.zjxtwvf.smarttime.utils.SpTools;
...
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, boolean isVerify,List<ImageView> pointViews,
		...
		paint.setColor(Color.rgb(00, 139, 00));// 设置默认连线颜色
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		for (int i = 0; i < 9; i++) {
			...
			int col = i % 3;
			// 定义点的每个属性
			int leftX = left+col*blockWidth+blockWidth/6;
			int topY = top+row*blockWidth+blockWidth/6;
			int rightX = left + col*blockWidth+blockWidth-blockWidth/6;
			int bottomY = top + row*blockWidth+blockWidth-blockWidth/6;
			GesturePoint p = new GesturePoint(leftX, rightX, topY, bottomY, mPointViews.get(i),i+1);
			this.list.add(p);
		}
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP:// 当手指抬起的时候
				if (isVerify) {
					...
					if (passWord == null || passWord.equals(passWordSb.toString())) {
						// 代表用户绘制的密码手势与传入的密码相同
